...
EtaUniversal <- function(width, n=2000) {
  ...
  for (i in 1:n) {
    ...
    traces[i] <- D.Matrix(spline.x=xi, only.trace=TRUE)
  }
  return (mean(traces))
}

#' @rdname eta
#' @param sigma (positive numeric) We do not care about vertical perturbations
#'     to within this amount.
#' @param dx.min (positive numeric) The closest allowable spacing between two
#'     spline knots.
#' @param dx.total (positive numeric) The distance between the first and last
#'     spline knots.
