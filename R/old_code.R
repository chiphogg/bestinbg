...
#' with itself.
#'
#' @param Phi  A (R x C) numeric matrix (usually a spline matrix, where the
#'     columns correspond to the knots, and the rows correspond to evaluation
#'     points).
#'
#' @return A 3-index numeric array, whose dimensions are c(C, C, R), such that
#'     the matrix at [*, *, i] is the outer product of Phi[i, ] with itself.
...
#'
#' @param H  a 3D numeric array; the 3rd index indicates which 2D matrix we're
#'     dealing with.
#' @param v  A vector for reducing H to a matrix.
#'
#' @return If v is a vector, returns the matrix M such that:
#'      \code{M[k, i] = sum_j H[i, j, k] v[j]}
...
#' would want this).
#'
#' @param DD  A D-matrix (giving overlap integrals for second derivatives of
#'      spline basis functions)
#'
#' @return The product of the largest all-but-two eigenvalues of DD (the
...
#'
#' @param cc  'c' in Fischer et al.: vector of spline knot y-values
#' @param d  vector of datapoints
#' @param Phi  spline matrix taking 'c' into 'd'
#' @param p.bgr  The probability that a single pixel contains "only"
#'     background.
#' @param sigma  vector of experimental uncertainties
...
#' @param sigma  vector of experimental uncertainties
#' @param lambda  mean signal magnitude for signal-containing pixels
#' @param calc.grad  If TRUE, also calculate the gradient
#' @param calc.hess  If TRUE, also calculate the Hessian
#'
#' @return A vector of values for the (signal-containing) likelihood of each
#'   datapoint.
...
#' hessian.
#'
#' @param params  The spline knot y-values ('c' in Fischer et al.).
#' @param x  The x-values of the measured datapoints.
#' @param y  The y-values of the measured datapoints.
#' @param xi  The spline knot x-values.
#' @param p.bgr  The probability that a single pixel contains "only" background.
